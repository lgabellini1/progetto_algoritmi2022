...
class AlphaBetaPro implements MNKPlayer {
    ...
    private double alphaBeta(MNKBoard B, boolean max_player, double alpha, double beta, int depth) {
        ...
        if (depth == 0 || B.gameState() != MNKGameState.OPEN)
            ...
            eval = -999;
            Map<Integer, ArrayList<MNKCell>> caselleConPriorita = max.getCelleConPriorita();
            for (int i = 8; i > 0; i--) {
                if (caselleConPriorita.get(i) != null) {
                    ArrayList<MNKCell> celleAlphaBeta = caselleConPriorita.get(i);
                    for (MNKCell cella : celleAlphaBeta) {
                        if ((System.currentTimeMillis() - start) / 1000.0 > TIMEOUT * (99.0 / 100.0))
                            break;

                        B.markCell(cella.i, cella.j);

                        current_hash = current_hash ^ zobristHash(cella, B);
                        max.update(cella, B);
                        min.update(cella, B);
            ...
            eval = +999;
            Map<Integer, ArrayList<MNKCell>> caselleConPriorita = min.getCelleConPriorita();
            for (int i = 8; i > 0; i--) {
                if (caselleConPriorita.get(i) != null) {
                    ArrayList<MNKCell> celleAlphaBeta = caselleConPriorita.get(i);
                    for (MNKCell cella : celleAlphaBeta) {
    ...
    private MNKCell parentAlphaBeta(MNKBoard B) {
        ...
        for (int i = 8; i > 0; i--) {
            if (caselleConPriorita.get(i) != null) {
                ArrayList<MNKCell> parentAlphaBeta = caselleConPriorita.get(i);
                for (MNKCell cella : parentAlphaBeta) {
    ...
    public MNKCell selectCell(MNKCell[] FC, MNKCell[] MC) {
        ...
        if (MC.length == 0) {
            ...
            B.markCell(c.i, c.j);
            // System.out.println(ANSI_RED + "Marked " + new
            // MNKCell(c.i,c.j,B.cellState(c.i,c.j)) + ANSI_RESET);
        ...
        if (MC.length == 1) {
            ...
            if (B.cellState(c.i, c.j) == MNKCellState.FREE) {
                ...
                MNKCell d = new MNKCell(B.M / 2, (B.N / 2) + 1);
                B.markCell(d.i, d.j);
        ...
        if (MC.length > 1) {
            ...
            min.update(c, B);
            // max.test_print(); min.test_print();
            starting_hash = current_hash; // modificato
        }
    ...
    public int cellRow(int cellIndex, MNKBoard B) {
        int i = 0;
        while (cellIndex >= B.M) {
            cellIndex = cellIndex - B.M;
            i = i + 1;
        }
        return i;
    }

    public int cellColumn(int cellIndex, MNKBoard B) {
        while (cellIndex >= B.M) {
            cellIndex = cellIndex - B.M;
        }
        return cellIndex;
